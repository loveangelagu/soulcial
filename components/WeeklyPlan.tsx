...
export function WeeklyPlan({
  ...
  if (!hasEventsThisWeek) {
    return (
      <div className="rounded-3xl border border-lavender-pale bg-paper/70 p-6 text-center">
        <p className="text-sm text-muted">no matches this week.</p>
        {onRelaxFilters && (
          <button
            type="button"
            onClick={onRelaxFilters}
            className="mt-2 text-purple-deep underline underline-offset-4 hover:text-purple-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-primary rounded"
          >
    ...
    <div className="mb-4 overflow-x-auto pb-1">
      <div className="flex w-full justify-center">
        <div
          ...
          className="flex min-w-max gap-2 snap-x snap-mandatory"
          onKeyDown={(e) => {
          ...
          {days.map((day, idx) => {
            ...
            return (
              <button
                ...
                className={[
                  'snap-start min-w-[72px] rounded-2xl border px-3 py-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-primary',
                  active
                    ? 'bg-lavender-light border-lavender text-purple-deep'
                    : 'bg-paper border-lavender-pale text-muted hover:text-purple-dark',
                  isToday ? 'ring-1 ring-lavender' : '',
                  isEmpty ? 'opacity-70' : '',
                ].join(' ')}
              >
                <div className="font-pixel text-xs uppercase">{formatBaliDay(day.date).slice(0, 3)}</div>
                <div className="font-pixel text-lg leading-tight">{day.date.getDate()}</div>
                <div className="h-2 mt-0.5">
                  {hasPinned && <span className="inline-block h-1.5 w-1.5 rounded-full bg-purple-deep" />}
                </div>
              </button>
            )
          })}
  ...
  return (
    <div className="mt-6 flex flex-col items-center gap-2">
      <button
        ...
        className="px-6 py-3 rounded-2xl bg-purple-deep text-white font-pixel text-xl shadow-[0_3px_0_0_#4a2a8a] hover:translate-y-0.5 hover:shadow-[0_1px_0_0_#4a2a8a] transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-primary"
      >
        add to my calendar →
      </button>
      <button
        type="button"
        onClick={onExportIcs}
        className="text-xs text-muted hover:text-purple-deep underline-offset-4 hover:underline transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-primary rounded"
      >
        download week plan (.ics)
      </button>
      {gcalStatus && (
        <p className="mt-1 text-xs text-purple-deep font-pixel" role="status" aria-live="polite">
          {gcalStatus}
        </p>
      )}
    </div>
...
function DaySpotlight({
  ...
  return (
    <section
      aria-live="polite"
      className="rounded-3xl border border-lavender-pale bg-gradient-to-b from-paper to-lavender-light/20 p-4 md:p-6 shadow-[0_4px_0_0_#e9e0ff]"
    >
      <header className="mb-4 flex items-center justify-between gap-2 border-b border-lavender-pale/70 pb-3">
        <h3 className="font-pixel text-xl md:text-2xl text-purple-dark">
          {day.date.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}
        </h3>
        {isToday && (
          <span className="rounded-full bg-lavender-light border border-lavender px-2 py-1 text-xs font-pixel text-purple-deep">
            today
          </span>
        )}
      </header>
...
function EventRow({
  ...
  return (
    <article
      className={[
        'rounded-2xl border bg-paper shadow-[0_2px_0_0_#efe8ff] p-3 transition-all hover:-translate-y-0.5',
        'border-l-[3px]',
    ...
    >
      <div className="flex items-start gap-3">
        <div className="shrink-0 font-pixel text-xs text-muted pt-1 min-w-[52px]">
        ...
        >
          <div className="font-pixel text-base text-purple-dark leading-tight">{event.name}</div>
        ...
        <button
          ...
          className={[
            'shrink-0 rounded-xl border px-2 py-1 text-xs font-pixel transition-colors',
      ...
      <div className="mt-2 pt-2 border-t border-lavender-pale/70 flex items-center justify-end">
        <a
          href={`https://nomeo.io/m/${event.uid}`}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center rounded-xl border border-lavender bg-lavender-light/40 px-2.5 py-1 text-xs font-pixel text-purple-deep hover:bg-lavender-light transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-primary"
        >
          view event page
        </a>
      </div>
    ...
    <div className="text-sm text-muted">
      ...
      {onRelaxFilters && (
        <button
          type="button"
          onClick={onRelaxFilters}
          className="mt-1 text-purple-deep underline underline-offset-4 hover:text-purple-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-primary rounded"
        >
          relax advanced filters
        </button>
      )}
...
function MyWeekTray({
  ...
  return (
    <>
      <div className="fixed bottom-4 left-1/2 z-20 w-[min(92vw,720px)] -translate-x-1/2 rounded-2xl border border-lavender bg-paper/95 shadow-[0_4px_0_0_#e9e0ff] p-3 flex items-center justify-between gap-3">
        <div className="text-sm text-purple-deep font-pixel">{pinnedCount} picks this week</div>
        <button
          type="button"
          onClick={onOpen}
          className="rounded-xl bg-purple-deep px-3 py-2 text-xs font-pixel text-white hover:translate-y-0.5 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-primary"
        >
          review
        </button>
      </div>
      {open && (
        <div className="fixed inset-0 z-30 bg-black/30 px-3 py-6 flex items-end md:items-center justify-center">
          <div className="w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-3xl border border-lavender bg-paper p-5">
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-pixel text-xl text-purple-dark">my week picks</h4>
              <button
                type="button"
                onClick={onClose}
                className="rounded-lg px-2 py-1 text-sm text-muted hover:text-purple-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-primary"
              >
                close
              </button>
            </div>
            <div className="space-y-4">
              {grouped.map(([dayKey, events]) => (
                <section key={dayKey}>
                  <div className="font-pixel text-sm text-purple-deep mb-1">
                  ...
                  <ul className="space-y-1">
                    {events.map((event) => (
                      <li key={event.uid} className="text-sm text-muted">
